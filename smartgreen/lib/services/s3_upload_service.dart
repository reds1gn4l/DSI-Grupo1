...
class S3UploadService {
  static Future<String> uploadFile({
    ...
    final key = '$s3DestDir/$filename';
    final endpoint = 'https://$s3Bucket.s3.$s3Region.amazonaws.com/$key';
    final now = DateTime.now().toUtc();
    final date = now.toIso8601String().substring(0, 10).replaceAll('-', '');

    // Corrigido: usando interpolação em vez de concatenação
    final isoString = now.toIso8601String();
    final withoutMicroseconds = isoString.split('.')[0];
    final amzDate =
        '${withoutMicroseconds.replaceAll('-', '').replaceAll(':', '')}Z';

    List<int> sign(List<int> key, String msg) =>
        Hmac(sha256, key).convert(utf8.encode(msg)).bytes;

    final kDate = sign(utf8.encode('AWS4$s3SecretKey'), date);
    final kRegion = sign(kDate, s3Region);
    final kService = sign(kRegion, 's3');
    final kSigning = sign(kService, 'aws4_request');

    final payloadHash = sha256.convert(await file.readAsBytes()).toString();
    final credentialScope = '$date/$s3Region/s3/aws4_request';
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception(
        'Failed to upload to S3: Status: ${response.statusCode} Body: ${response.data}',
      );
    }
  }
}
