...
class _ProductFormPageState extends State<StoreProductFormPage> {
  final _cientificNameCtrl = TextEditingController();
  ...
  final _umidadeMaxCtrl = TextEditingController();
  final _umidadeMinCtrl = TextEditingController();
  ...
  Future<void> _save() async {
    ...
    double? tempMax = double.tryParse(_tempMaxCtrl.text.replaceAll(',', '.'));
    double? tempMin = double.tryParse(_tempMinCtrl.text.replaceAll(',', '.'));
    double? umiMax = double.tryParse(_umidadeMaxCtrl.text.replaceAll(',', '.'));
    double? umiMin = double.tryParse(_umidadeMinCtrl.text.replaceAll(',', '.'));
    String fxTemp =
        (tempMax != null && tempMin != null)
            ? ((tempMax + tempMin) / 2).toStringAsFixed(1)
            : '';
    String fxUmidade =
        (umiMax != null && umiMin != null)
            ? ((umiMax + umiMin) / 2).toStringAsFixed(1)
            : '';

    final p = StoreProduct(
      id: widget.storeProduct?.id ?? '',
      nome: _nameCtrl.text.trim(),
      cientificName: _cientificNameCtrl.text.trim(),
      ...
      category: _catValue?.isEmpty ?? true ? null : _catValue,
      imageURL: _imageUrl ?? '',
              ...
              child: Column(
                children: [
                  ...
                  if (_catValue != 'Insumo/Ferramenta') ...[
                    TextFormField(
                      controller: _cientificNameCtrl,
                      decoration: _dec('Nome Científico'),
                    ),
                    const SizedBox(height: 12),
                  ],
                  // Categoria
                  DropdownButtonFormField<String>(
                    ...
                    items: const [
                      DropdownMenuItem(
                        value: 'Planta/Semente',
                        child: Text('Planta/Semente'),
                      ),
                      DropdownMenuItem(
                        value: 'Insumo/Ferramenta',
                        child: Text('Insumo/Ferramenta'),
                      ),
                    ],
                    onChanged:
                        _saving ? null : (v) => setState(() => _catValue = v),
                    validator:
                        (v) =>
                            (v == null || v.isEmpty)
                      ...
                      children: [
                        Expanded(
                          child: TextFormField(
                            ...
                            decoration: _dec('Temperatura Mínima (°C)'),
                        ...
                        Expanded(
                          child: TextFormField(
                            ...
                            decoration: _dec('Temperatura Máxima (°C)'),
                            keyboardType: const TextInputType.numberWithOptions(
                              decimal: true,
                            ),
                          ),
                  ...
                  if (_catValue != 'Insumo/Ferramenta') ...[
                    Row(
                      children: [
                        Expanded(
                          child: TextFormField(
                            controller: _umidadeMinCtrl,
                            decoration: _dec('Umidade Mínima (%)'),
                            keyboardType: const TextInputType.numberWithOptions(
                              decimal: true,
                            ),
                          ),
                        ...
                        Expanded(
                          child: TextFormField(
                            controller: _umidadeMaxCtrl,
                            decoration: _dec('Umidade Máxima (%)'),
                            keyboardType: const TextInputType.numberWithOptions(
                              decimal: true,
                            ),
                          ),
    ...
    return Container(
      ...
      child: FittedBox(
        ...
        child: Column(
          ...
          children: [
            const LeafGlyph(size: 40, color: Colors.grey),
